...
export default function VillaShow({ villa }: VillaShowProps) {
    return (
        <AppLayout
            breadcrumbs={[
                                ...
                                {villa.completion_pct != null ? (
                                    <div className="flex items-center gap-2">
                                        <div className="bg-muted h-2 w-24 overflow-hidden rounded-full">
                                            <div
                                                className="h-full rounded-full bg-mbp-gold transition-all"
                                                style={{ width: `${Math.min(villa.completion_pct, 100)}%` }}
                                            />
                                        </div>
                                        <span>{villa.completion_pct.toFixed(0)}%</span>
                                    </div>
                                ) : (
                                    '-'
                                )}
                            </InfoRow>
                            <InfoRow label="Sale Date">{formatDate(villa.sale_date)}</InfoRow>
                        </dl>
                    </CardContent>
                </Card>
                ...
                <Tabs defaultValue="details">
                    <TabsList>
                        ...
                        <TabsTrigger value="updates" className="gap-1.5">
                            ...
                            Site Updates
                            <Badge variant="secondary" className="ml-1 text-xs">
                                {villa.villa_site_updates.length}
                            </Badge>
                        </TabsTrigger>
                    </TabsList>
                                ...
                                <CardContent>
                                    <dl className="grid grid-cols-1 gap-4">
                                        ...
                                        <InfoRow label="Structural Status">
                                            <StatusBadge status={villa.structural_status} />
                                        </InfoRow>
                                        <InfoRow label="Finishing Status">
                                            <StatusBadge status={villa.finishing_status} />
                                        </InfoRow>
                                        <InfoRow label="Facade Status">
                                            <StatusBadge status={villa.facade_status} />
                                        </InfoRow>
                                        <InfoRow label="Current Stage">
                                            {villa.current_stage?.name ?? '-'}
                                        </InfoRow>
                                    </dl>
                                </CardContent>
                            </Card>
                                    ...
                                    <dl className="grid grid-cols-1 gap-4">
                                        <InfoRow label="Planned Start">
                                            <span className="inline-flex items-center gap-1.5">
                                                ...
                                                {formatDate(villa.planned_start)}
                                            </span>
                                        </InfoRow>
                                        <InfoRow label="Planned Finish">
                                            <span className="inline-flex items-center gap-1.5">
                                                <Calendar className="text-muted-foreground h-3.5 w-3.5" />
                                                {formatDate(villa.planned_finish)}
                                            </span>
                                        </InfoRow>
                                        <InfoRow label="Actual Start">
                                            <span className="inline-flex items-center gap-1.5">
                                                <Calendar className="text-muted-foreground h-3.5 w-3.5" />
                                                {formatDate(villa.actual_start)}
                                            </span>
                                        </InfoRow>
                                        <InfoRow label="Actual Finish">
                                            <span className="inline-flex items-center gap-1.5">
                                                <Calendar className="text-muted-foreground h-3.5 w-3.5" />
                                                {formatDate(villa.actual_finish)}
                                            </span>
                                        </InfoRow>
                                        <InfoRow label="Accumulated Concrete">
                                            {villa.acc_concrete_qty != null
                                                ? `${villa.acc_concrete_qty.toLocaleString()} m³`
                                                : '-'}
                                        </InfoRow>
                                        <InfoRow label="Accumulated Steel">
                                            {villa.acc_steel_qty != null
                                                ? `${villa.acc_steel_qty.toLocaleString()} kg`
                                                : '-'}
                                        </InfoRow>
                                    </dl>
                                </CardContent>
                            ...
                            <div className="relative space-y-6 pl-6 before:absolute before:top-0 before:bottom-0 before:left-2 before:w-px before:bg-border">
                                {villa.villa_site_updates.map((update) => (
                                    <div key={update.id} className="relative">
                                        <div className="bg-mbp-gold absolute -left-6 top-1 h-4 w-4 rounded-full border-2 border-white dark:border-gray-900" />
                                        <Card>
                                            <CardHeader className="pb-2">
                                                <CardTitle className="flex items-center gap-2 text-sm">
                                                    <Calendar className="h-4 w-4 text-mbp-gold" />
